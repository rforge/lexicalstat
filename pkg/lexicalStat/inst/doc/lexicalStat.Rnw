...
w <- wam(asFrequencyList(c), subcorpus=asFrequencyList(sc), measure="loglikelihood");
...
scl <- asFrequencyList(sc, "word");
cl <- asFrequencyList(tabulated, "word")

w <- wam(cl, subcorpus=scl, measure="loglikelihood");
...
\texttt{frequency}. It can be used as any other data frame. Some other functions are
provided : \texttt{N}\index{N}, \texttt{V}\index{V}, \texttt{summary}\index{summary}
and \texttt{types}\index{types}.
...
You can also ask for the frequency of a given type with
\texttt{frequencies}\index{frequencies}
or ask if a given type is available in this frequency list with
\texttt{has.type}\index{have.type}

<<frequencyList_other_functions, echo=T>>=
frequencies(dickensFrequencyList, c("the", "."))
has.types(dickensFrequencyList, c("the", "blabla"))
@
...
Here is the first lines for the \texttt{robespierre} lexical table, giving the frequency
...
data(robespierre);
x <- lexicalTable(robespierre);
w <- wam(x, measure=c("specificities", "loglikelihood"))
print(w, from=1, to=5)
@
