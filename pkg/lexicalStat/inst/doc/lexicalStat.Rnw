...
wam(asFrequencyList(c), subcorpus=asFrequencyList(sc));
@
...
actual corpora (\pageref{subsec:read-corpora}) and for converting from one format
toward another (\pageref{sec:classes}).
