...
fileName <- system.file(c("exempleData"),
                        "LeMondeEco.lines",
                        package="lexicalStat")
c <- readTexts(fileName, parts="sentences");
...
dir <- system.file(c("exempleData", "zola"),
                        package="lexicalStat")
c <- readTexts(dir=dir, parts="files");
wam(c);
@
...
All object support three functions:
\begin{itemize}
\item \texttt{N}\index{N} : the number of tokens
\item \texttt{V}\index{V} : the number of different types (V stand for vocabulary)
\item \texttt{types}\index{types} : the sorte list of the different types.
\end{itemize}

The function \texttt{summary}\index{summary} applied on this data structures provide
short resumes of their content.

For each of these four types, a tool corpus is provided as a dataset (load with \texttt{data}).
These tool corpus represent all the two first sentences of "A Chrismas x" by Ch. Dickens. It is:
...
Functions are provided for converting from one format toward another.

Functions are provided for reading from files these data.
...
% ----------------------------------------------------------------
\subsection{Functions available for all classes}
% ----------------------------------------------------------------
\label{subsec:functions-corpora}

All classes provide the following functions :
\begin{itemize}
\item \texttt{N}\index{N} The grand total of tokens in the corpus
\item \texttt{V}\index{V} The number of differents types in the corpus
\item \texttt{hapax}\index{hapax} the list of types which appears only once in the corpus
\item \texttt{types}\index{types} the list of all types in the corpus.
\end{itemize}

<<communfunctions1, echo=T>>=
@

A positional attribute argmuent must be provided with the functions \texttt{V},
\texttt{hapax}, \texttt{types} for a \texttt{tabulated} object.

<<communfunctions2, echo=T>>=
@
